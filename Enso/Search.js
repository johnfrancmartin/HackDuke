...
class Search extends Component {
  ...
  async componentWillMount(){
    var sourceChoices = sourceFinder.getAllSources();
    var sources = []
  ...
  getRows(numRows){
    let array = [];
    var undisplayedNews = this.state.undisplayedNews.slice(0);
    var undisplayedVideos = this.state.undisplayedVideos.slice(0);
  ...
  onChangeText(searchTerm){
    ...
    let articles = this.state.news.slice(0);
    let newsResults = [];
    for (i = 0; i < articles.length; i++){
      let article = articles[i];
      let headline = article.headline.toLowerCase();
      let description = article.description.toLowerCase();
      if (headline.includes(term) || description.includes(term)){
        newsResults.push(article);
      }
    }
    let videos = this.state.videos.slice(0);
    let videoResults = [];
    ...
    this.setState({undisplayedNews: newsResults, undisplayedVideos: videoResults})
  }

  render() {
    let listWidth = 0.9*Dimensions.get('window').width;
    let listHeight = 0.9*Dimensions.get('window').height;
    ...
    if (!this.state.loaded){
      return (
        ...
        return (
          <View style = {{flex: 1, maxHeight: listHeight, minWidth: listWidth, maxWidth: listWidth}}>
              <SearchBar
                onSearchChange={(event) => this.onChangeText(event.nativeEvent.text)}
                height={50}
                width={100}
