...
class Preferences extends Component {
  ...
  render() {
    ...
    let karmaRatio = this.state.karma;
    let karmaRatioString = karmaRatio.toFixed(4);
    let padding = 10;
    if (this.state.loaded){
      return(
        <View style = {{position: 'absolute', alignItems: 'center', top: 200, left: 0.15*Dimensions.get('window').width - viewWidth/2,
        width: 0.7*Dimensions.get('window').width, borderRadius: 10, height: checkboxHeight*keysLength+2*padding+karmaHeight, backgroundColor: 'white'}}>
          <View style = {{height: padding}}/>
          <Text>{"Karma Ratio: " + karmaRatioString}</Text>
          <View style = {{height: padding}}/>
          <ListView
